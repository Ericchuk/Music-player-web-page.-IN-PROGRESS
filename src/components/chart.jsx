import more from "./images/more-vertical.svg";
import leadImage from './images/Lead-image.png'
import audioImg1 from "./images/Rectangle 14.svg";
import audioImg2 from "./images/Rectangle 15.svg";
import audioImg3 from "./images/Rectangle 16.svg";
import audioImg4 from "./images/Rectangle 17.svg";
import audioImg5 from "./images/Rectangle 18.svg";
import audioImg6 from "./images/Rectangle 19.svg";
import audioImg7 from "./images/Rectangle 20.svg";

export default function Chart() {
  const songs = [
    {
      id: 0,
      img: audioImg1,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 1,
      img: audioImg2,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 2,
      img: audioImg3,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 3,
      img: audioImg4,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 4,
      img: audioImg5,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 5,
      img: audioImg6,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 6,
      img: audioImg7,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
    {
      id: 7,
      img: audioImg7,
      title: "Let me love you - Krisx",
      artist: "Single",
      duration: "4:17",
    },
  ];

  //loop over each item and return a aside element that is made of the object in songs

  const eachSong = songs.map((item) => {
    return (
      <aside key={item.id}>
        <div>
          <img src={item.img} alt={item.title} className="img" />
          <div>
            <h3>{item.title}</h3>
            <p>{item.artist}</p>
          </div>
        </div>
        <div className="duration">
          <img src={more} alt="more" />
          <p>{item.duration}</p>
        </div>
      </aside>
    );
  });

  return (
    <section className="chart">
      <section>
        <img src={leadImage} alt="lead-image" />
        <aside>
          <h2>Tomorrow's tunes</h2>
          <p>
            Listen to distinct and well articulated sounds. Vibe to the likes
            of Burna-boy, Wizkid and others
          </p>
          <p>64 songs ~ 16 hrs+</p>
          <aside>
            <div>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="Play">
                <path
                  id="Vector"
                  d="M7.00004 0.333374C10.6744 0.333374 13.6667 3.32421 13.6667 7.0039C13.6667 10.6759 10.6744 13.6667 7.00004 13.6667C3.32565 13.6667 0.333374 10.6759 0.333374 7.0039C0.333374 3.32421 3.32565 0.333374 7.00004 0.333374ZM6.24071 4.35331C6.09914 4.35331 5.964 4.38547 5.8353 4.44979C5.67443 4.53984 5.54573 4.68134 5.47494 4.84857C5.4299 4.96434 5.35911 5.31167 5.35911 5.3181C5.28833 5.69758 5.24972 6.31504 5.24972 6.99682C5.24972 7.64709 5.28833 8.23818 5.34624 8.6241C5.35268 8.63053 5.42346 9.06146 5.50068 9.2094C5.64225 9.47954 5.91896 9.64677 6.21497 9.64677H6.24071C6.43376 9.64034 6.83916 9.47311 6.83916 9.46667C7.52128 9.18367 8.86619 8.3025 9.40673 7.7172L9.44534 7.67861C9.51613 7.60786 9.60622 7.49851 9.62552 7.47278C9.72848 7.33771 9.77996 7.17049 9.77996 7.0039C9.77996 6.81673 9.72205 6.64307 9.61265 6.50157C9.58691 6.47584 9.49039 6.3665 9.4003 6.27645C8.87263 5.71044 7.49554 4.78425 6.77481 4.50125C6.66542 4.45687 6.38871 4.35974 6.24071 4.35331Z"
                  fill="#FACD66"
                />
              </g>
            </svg>

            <small>Play all</small>
          </div>
          <div>
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="music-square-add">
                <path
                  id="Vector"
                  d="M12.8067 10.0067C12.08 9.96002 11.3333 10.2267 10.78 10.78C9.74 11.82 9.74 13.5067 10.78 14.5534C11.1333 14.9067 11.5667 15.14 12.0267 15.2534C12.28 15.32 12.5467 15.34 12.8133 15.3267C13.4467 15.3 14.0667 15.0467 14.5533 14.56C15.24 13.8734 15.4733 12.9067 15.2533 12.0334C15.1467 11.5734 14.9067 11.14 14.5533 10.7867C14.0733 10.3 13.4467 10.04 12.8067 10.0067ZM14.16 12.6534C14.16 12.7934 14.1067 12.9134 14.0133 13.0067C13.92 13.1 13.8 13.1534 13.66 13.1534H13.1667V13.6734C13.1667 13.8134 13.1133 13.9334 13.02 14.0267C12.9267 14.12 12.8067 14.1734 12.6667 14.1734C12.3933 14.1734 12.1667 13.9467 12.1667 13.6734V13.1534H11.6667C11.3933 13.1534 11.1667 12.9267 11.1667 12.6534C11.1667 12.38 11.3933 12.1534 11.6667 12.1534H12.1667V11.68C12.1667 11.4067 12.3933 11.18 12.6667 11.18C12.94 11.18 13.1667 11.4067 13.1667 11.68V12.1534H13.66C13.94 12.1534 14.16 12.38 14.16 12.6534Z"
                  fill="#FACD66"
                />
                <path
                  id="Vector_2"
                  d="M8.15337 8.04675C7.88003 8.04675 7.66003 8.26675 7.66003 8.54009C7.66003 8.81342 7.88003 9.03342 8.15337 9.03342C8.4267 9.03342 8.6467 8.81342 8.6467 8.54009C8.6467 8.26675 8.4267 8.04675 8.15337 8.04675Z"
                  fill="#FACD66"
                />
                <path
                  id="Vector_3"
                  d="M4.50664 8.70667C4.23331 8.70667 4.01331 8.92666 4.01331 9.2C4.01331 9.47333 4.23331 9.69333 4.50664 9.69333C4.77997 9.69333 4.99997 9.47333 4.99997 9.2C4.99997 8.92666 4.77997 8.70667 4.50664 8.70667Z"
                  fill="#FACD66"
                />
                <path
                  id="Vector_4"
                  d="M10.7934 1.33337H5.20671C5.02004 1.33337 4.84004 1.34004 4.66671 1.36671C2.56671 1.56004 1.33337 2.96671 1.33337 5.20671V10.7934C1.33337 13.0334 2.56671 14.44 4.66671 14.6334C4.84004 14.66 5.02004 14.6667 5.20671 14.6667H9.00004C9.26004 14.6667 9.42671 14.3734 9.32671 14.1334C9.13337 13.6734 9.00004 13.14 9.00004 12.6667C9.00004 10.6467 10.6467 9.00004 12.6667 9.00004C13.1734 9.00004 13.6667 9.10004 14.12 9.30004C14.3667 9.40671 14.6667 9.24004 14.6667 8.97337V5.20671C14.6667 2.78004 13.22 1.33337 10.7934 1.33337ZM9.65337 5.36671V8.54004C9.65337 8.54671 9.64671 8.55337 9.64671 8.56671C9.63337 9.38004 8.97337 10.04 8.15337 10.04C7.32671 10.04 6.66004 9.36671 6.66004 8.54671C6.66004 7.72004 7.33337 7.05337 8.15337 7.05337C8.32671 7.05337 8.49337 7.08671 8.65337 7.14671V6.02004L6.00671 6.74004V9.20671C6.00671 9.21337 6.00004 9.22004 6.00004 9.22671C5.98671 10.04 5.32671 10.6934 4.50671 10.6934C3.68004 10.6934 3.01337 10.02 3.01337 9.20004C3.01337 8.38004 3.68671 7.70671 4.50671 7.70671C4.68004 7.70671 4.84671 7.74004 5.00004 7.80004V6.36004V5.19337C5.00004 4.57337 5.38671 4.07337 5.98004 3.91337L7.96671 3.36671C8.58671 3.20004 8.96671 3.36671 9.18004 3.52671C9.48671 3.76004 9.64004 4.14004 9.64004 4.64671V5.36671H9.65337Z"
                  fill="#FACD66"
                />
              </g>
            </svg>

            <small>Add to collection</small>
          </div>
          <div>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="Heart">
                <path
                  id="Vector"
                  d="M4.51993 0.666973C4.93993 0.679746 5.3466 0.75308 5.7406 0.88708H5.77993C5.8066 0.899746 5.8266 0.913746 5.83993 0.926413C5.98726 0.973746 6.1266 1.02708 6.25993 1.10041L6.51326 1.21375C6.61326 1.26708 6.73326 1.36641 6.79993 1.40708C6.8666 1.44641 6.93993 1.48708 6.99993 1.53308C7.7406 0.96708 8.63993 0.660413 9.5666 0.666973C9.98726 0.666973 10.4073 0.726413 10.8066 0.860413C13.2673 1.66041 14.1539 4.36041 13.4133 6.72041C12.9933 7.92641 12.3066 9.02708 11.4073 9.92641C10.1199 11.1731 8.70726 12.2797 7.1866 13.2331L7.01993 13.3337L6.8466 13.2264C5.3206 12.2797 3.89993 11.1731 2.6006 9.91975C1.70726 9.02041 1.01993 7.92641 0.593263 6.72041C-0.16007 4.36041 0.726597 1.66041 3.21393 0.846413C3.40726 0.779746 3.6066 0.73308 3.8066 0.70708H3.8866C4.07393 0.679746 4.25993 0.666973 4.4466 0.666973H4.51993ZM10.4599 2.77375C10.1866 2.67975 9.8866 2.82708 9.7866 3.10708C9.69326 3.38708 9.83993 3.69375 10.1199 3.79308C10.5473 3.95308 10.8333 4.37375 10.8333 4.83975V4.86041C10.8206 5.01308 10.8666 5.16041 10.9599 5.27375C11.0533 5.38708 11.1933 5.45308 11.3399 5.46708C11.6133 5.45975 11.8466 5.24041 11.8666 4.95975V4.88041C11.8866 3.94641 11.3206 3.10041 10.4599 2.77375Z"
                  fill="#E5524A"
                />
              </g>
            </svg>

            <small>Like</small>
          </div>
          </aside>
        </aside>
      </section>

      <section className="songs">{eachSong}</section>
    </section>
  );
}
